...
DirCheck::DirCheck()
{
    m_message = "please prepare...";
}

QString DirCheck::message() const{
    return m_message;
}

void DirCheck::setMessage(const QString msg){
    m_message = msg;
}

QString DirCheck::getDestationDirector(QString destDire){
    m_destDirector = destDire.mid(8,destDire.length()-8);
    return m_destDirector;
}
QString DirCheck::getResourceDirector(QString resoDire){
    m_resoDirector = resoDire.mid(8,resoDire.length()-8);
    return m_resoDirector;
}

void DirCheck::test(){
    qDebug() << "m_destDirector=" << m_destDirector;
    qDebug() << "m_resoDirector=" << m_resoDirector;
//    QString fileName = "sss.xlsx";
//    QString absuExcelPath=createFile(m_destDirector,fileName);
//     qDebug() << "test::" << absuExcelPath;
}
...
void DirCheck::readXmlFile(const QString &fileName){
    ...
    while (!reader.atEnd())
    {
        if (reader.isStartElement()){
            if (reader.name() == "TS"){
                int numb=2;
                if(reader.attributes().value("language")=="en_AU"){
                   ...
                   readContextlMembers("A",numb);
                    ...
                    qDebug() << "can't find the en_AU/zh_CN/ja_JP";
                    break;
                }
            }else{
